...
public class Day17Part2 {
    ...
    static int dijkstra(Set<Position> visited, PriorityQueue<State> next) {
        ...
        while (true) {
            nextState = next.poll();
            if (nextState.pos.coord.r == grid.length - 1 && nextState.pos.coord.c == grid[0].length - 1) {
                if (nextState.pos.run < 4) {
                    continue;
                } else {
                    break;
                }
            ...
            if (nextState.pos.run < 4) {
                dirs = Set.of(nextState.pos.last);
            } else if (nextState.pos.run >= 10) {
