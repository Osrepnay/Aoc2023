...
public class Day16Part2 {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for (int r = 0; r < maxRows; r++) {
            maxEnergized = Math.max(maxEnergized, energize(new At(new Position(r, 0), Direction.RIGHT)));
            maxEnergized = Math.max(maxEnergized, energize(new At(new Position(r, maxCols - 1), Direction.LEFT)));
        }
        for (int c = 0; c < maxCols; c++) {
            maxEnergized = Math.max(maxEnergized, energize(new At(new Position(0, c), Direction.DOWN)));
            maxEnergized = Math.max(maxEnergized, energize(new At(new Position(maxRows - 1, c), Direction.UP)));
        }
